...
export default async function PatientWeekPage({
  ...
  if (view === "prev") {
    ...
    rangeStart = new Date(mondayThisWeek);
    rangeEnd = new Date(mondayThisWeek);
    rangeEnd.setDate(mondayThisWeek.getDate() + 6);
  }
    ...
    <main className="max-w-md mx-auto px-4 py-6 pb-24">
      <header className="mb-4">
        ...
        <p className="text-xs text-neutral-500">
          {view === "prev" ? "Semana anterior · lunes a domingo" : "Esta semana · lunes a domingo"}
        </p>
      </header>
      ...
      <div className="space-y-3">
        {days.map(({ date, key, sessions: daySessions }) => {
          const dow = date.getDay() === 0 ? 7 : date.getDay();
          const isToday = date.toDateString() === new Date().toDateString();
          return (
            <div key={key} className={`card ${isToday ? "border-neutral-900" : ""}`}>
